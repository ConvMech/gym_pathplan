...
def nature_cnn1d_3(scaled_images, **kwargs):
	layer1 = tf.layers.conv1d(scaled_images, filters=20, kernel_size=4, strides=2, 
...
class CustomPolicy3(ActorCriticPolicy):
	def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse=False, **kwargs):
		super(CustomPolicy3, self).__init__(sess, ob_space, ac_space, n_env, n_steps, n_batch, n_lstm=256,
										   reuse=reuse, scale=True)

		with tf.variable_scope("model", reuse=reuse):
			activ = tf.nn.relu

			extracted_features = nature_cnn1d_3(self.processed_x, **kwargs)
			extracted_features = tf.layers.flatten(extracted_features)
