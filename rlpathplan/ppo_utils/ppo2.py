...
def render(*, policy, env, nsteps, episodes, ent_coef=0.0,
    vf_coef=0.5,  max_grad_norm=0.5, nminibatches=4, callback=None):
    ...
    make_model = lambda : Model(policy=policy, ob_space=ob_space, ac_space=ac_space, 
        nbatch_act=nenvs, nbatch_train=nbatch_train, nsteps=nsteps, ent_coef=ent_coef, 
    ...
    # load obs_rms
    import pickle
    with open(callback.pickledir + '/ob_rms.pkl', 'rb') as output:
...
def render_object(*, policy, env, nsteps, episodes, ent_coef=0.0,
    vf_coef=0.5,  max_grad_norm=0.5, nminibatches=4, callback=None):
    ...
    assert len(env.venv.envs) == 1
    env_test = env.venv.envs[0]
    env_test.env._max_episode_steps = 5000
    for e in range(episodes):
        velrecord = []
        obsrecord = []
        done = False
        rsum = 0
        rand_index = e
        objsize = np.load(OBJDIR + '/objsize_{}.npy'.format(rand_index))
        objangle = np.load(OBJDIR + '/angle_{}.npy'.format(rand_index))
        assert hasattr(env_test.unwrapped, 'objsize')
        env_test.unwrapped.objsize = objsize
        env_test.unwrapped.objangle = objangle
        obs = env_test.reset(test=1)
        obs = filter(obs)
        steps = 0
        zerosvel = 0
        if episodes < 11:
...
def collect_samples_evaluate(*, policy, env, nsteps, 
    total_timesteps, ent_coef, lr, 
    vf_coef=0.5,  max_grad_norm=0.5, gamma=0.99, lam=0.95, 
    log_interval=10, nminibatches=4, noptepochs=4, cliprange=0.2,
    save_interval=0, callback=None):
